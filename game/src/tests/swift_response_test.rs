...
use crate::{tests::common_test::test_state_w_decks, event::EventResult};
...
async fn test_gagglemaster() -> Result<()> {
    ...
    let move_bat=game.move_zones(library[1], Zone::Library, Zone::Battlefield).await;
    ...
    let hand=game.draw(pl).await[0];
    {
        ...
        assert!(!swift.effect.iter().any(|x|game.is_valid_target(x, hand, watchdog.into(), Zone::Battlefield)));
    }
